/*
 ...
 Copyright : Universit√© de Sherbrooke
...
void buildH(int32c *, int32c *, int32c *, int32c *, int32c *, int32c *, int);
        ...
        if (inputBufferFull) {
            ...
            if (SWT_GetValue_Local(0) == 0) { // MAIN DECISION TREE: PASS-THROUGH
                ...
                for(k=0; k < FFT_LEN; k++)
                {
                    re = outFFT[k].re;
                    im = outFFT[k].im; 
                    debugBuffer1[k] = 10*log((pow(re,2) + pow(im, 2) + 1));
                }
                ...
                // Test -> signal entrÈe = 2000 hz| rÈsultat obtenue maxAmplFreq = 1996 Hz, 2011Hz en debug
                // afficher 7segment est quasiment +- 10hz de diffÈrence de la frÈquence du signal ‡ l'entrÈe
...
//                Tester la rÈponse impulsionnelle
//                for (n = 0; n < FFT_LEN; n++) {
...
void buildH(int32c *H7p, int32c *H6p, int32c *H5p, int32c *H4p, int32c *H3p, int32c *Hp, int N) {
    int n;
